...
function git-big {
  ...
  limit=${1-10}
  grep --color=never -f <(git verify-pack -v $(git rev-parse --git-dir)/objects/pack/*.idx | grep -E "^\w+ blob\s+\d+ \d+ \d+$" | sort -k 3 -rn | head -$limit | awk '{print $1}') <(git rev-list --all --objects)
}
